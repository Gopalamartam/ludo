...
class ApiController extends Controller
{
	...
	public function payamount($orderid)
	{
		...
		if(!empty($orderDetail))
		{
			...
			$hash=hash('sha512', $config['merchantkey'].'|'.$txnId.'|'.$orderDetail->amount.'|batchamount|'.$user->name.'|'.$user->email.'|||||'.$data->udf5.'||||||'.$config['salt']);
			return view('payment',compact("config",'orderDetail','txnId','user'));
		}
		else{

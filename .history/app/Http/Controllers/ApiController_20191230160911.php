...
class ApiController extends Controller
{
	...
	public function getQuestion(Request $request)
	{
		...
		$allother = Quiz::where("date", "<", date("Y-m-d"))->where("status", 1)->get();
		if (!empty($todayquiz)) {
		...
		if (!empty($questiontoday[0])) {
			...
			foreach ($questiontoday as $key => $value) {
				...
				$result[$key]['quizeopen'] = $open;

				$result[$key]['correct'] = $value->correct;
				$result[$key]['ansgive'] = $ansGive;
				$result[$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result[$key]['date'] = date("d-m-Y",strtotime($quizess->date));
			}
		}
		...
		if (!empty($questionyesterday[0])) {
			...
			foreach ($questionyesterday as $key => $value) {
				...
				$result1[$key]['quizeopen'] = 1;
				
				$result1[$key]['correct'] = $value->correct;
				$result1[$key]['ansgive'] = $ansGive;
				$result1[$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result1[$key]['date'] = date("d-m-Y",strtotime($quizess->date));
			}
		}
		$result2 = array();
		if (!empty($allother[0])) {
			foreach ($allother as $key1 => $value1) {
				...
				if ($requireQuestion <= $totalquestion) {
					...
					$result2[$key1]['date'] = date("d-m-Y",strtotime($value1->date));
					$tempArray = array();
	...
	public function storeQuestion(Request $request)
	{
		...
		if (!empty($userid) && !empty($questionId) && !empty($givenAns)) {
			...
			$orderData['id']=$orders->id;
			$txnId="txnId_".rand(111111111111,999999999999)."_".$orders->id;
			...
			$orderData['id']=$orders->id;
			$orderData['txnid']=$orders->txn_id;
			$orderData['amount']=$orders->amount;
			return response()->json([
				'statusCode' => 200,
				'message' => $message,
				'data' => array("userid" => $userid,"url"=>$url,"userData"=>$userData,"orderData"=>$orderData),
			]);
		} else {
			$message = "Wrong Parameter Passed";
			return response()->json([
	...
	public function prize(Request $request)
	{
		...
		if (!empty($userprize[0])) {
			...
			foreach ($userprize as $key => $val) {
				...
				$result[$key]['date'] = date("d-m-Y",strtotime($val->date));
			}
	...
	public function withdrawal(Request $request)
	{
		...
		if (!empty($WithdrawalRequestdata[0])) {
			foreach ($WithdrawalRequestdata as $key => $val) {
				...
				$withdral[$key]['date'] =date("d-m-Y",strtotime($val->date));
	...
	public function historyDetails(Request $request)
	{
		...
		foreach ($questionyesdaydata as $key3 => $value3) {
			...
			$result['questions'][$key3]['date'] = date("d-m-Y",strtotime($value3->date));
	...
	public function responsehash(Request $request)
	{
		$config=Config::get('constants');
		...
		if($calculated_hash==$hash)
		{
			...
			return response()->json([
				'statusCode' => 200,
				'message' => $message,
				'data' => array("hash"=>$calculated_hash),
			]);
		}
		else
		{

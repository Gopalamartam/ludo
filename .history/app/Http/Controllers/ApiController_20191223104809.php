...
class ApiController extends Controller
{
	...
	public function register(Request $request)
	{
		...
		if (!empty($name)  && !empty($email) && !empty($phone) && !empty($password)) {
			...
			if (!empty($usercheck)) {
				...
				$returndata = array();
				$returndata['userId'] = $user->id ? $user->id : '';
				$returndata['name'] = $user->name ? $user->name : '';


				$returndata['phone'] = $user->phone ? $user->phone : '';

				$returndata['email'] = $user->email ? $user->email : '';
				$returndata['otp'] =	$otp ? $otp : '';

				$returndata['bank'] = $user->name ? $user->name : '';
				$returndata['acount_no'] = $user->phone ? $user->phone : '';

				$returndata['ifsc'] = $user->email ? $user->email : '';
				$returndata['paytm'] =	$otp ? $otp : '';
				$returndata['image'] = $user->image ? $user->image : '';;
				$returndata['refercode'] = ($user->refercode) ?  $user->refercode : '';
				$returndata['wallet'] = ($user->wallet) ? $user->wallet : '';
	...
	public function otpMatch(Request $request)
	{
		...
		if ($user) {
			...
			$returndata['email'] = $user->email ? $user->email : '';
				$returndata['bank'] = $user->bank ? $user->bank : '';
				$returndata['acount_no'] = $user->acount_no ? $user->acount_no : '';

				$returndata['ifsc'] = $user->ifsc ? $user->ifsc : '';
				$returndata['paytm'] =	$user->paytm ? $user->paytm : '';
			$returndata['otp'] =	$user->email ? $user->email : '';
			$returndata['image'] = $user->image ? $user->image : '';
			$returndata['refercode'] = ($user->refercode) ?  $user->refercode : '';
	...
	{
		User::where("id", $userid)->update(['otp_match' => $otp]); 
			$user = User::where('phone', '=', $phone)->first();
			$data = array(
				'userId' => ($user->id) ? $user->id : '',
				'name' => ($user->name) ? $user->name : '',
	...
	public function updateProfile(Request $request)
	{
		...
		if (!empty($checkUser)) {
			if (!empty($name)) {
				...
				$userArray = array();
				$userArray['userId'] 	= ($userData->id) ? $userData->id : '';
				$userArray['name'] 	= ($userData->name) ? $userData->name : '';

				$userArray['email'] = ($userData->email) ? $userData->email : '';
				$userArray['phone'] = ($userData->phone) ? $userData->phone : '';
				$userArray['bank'] = $userData->bank ? $userData->bank : '';
				$userArray['acount_no'] = $userData->acount_no ? $userData->acount_no : '';

				$userArray['ifsc'] = $userData->ifsc ? $userData->ifsc : '';
				$userArray['paytm'] =	$userData->paytm ? $userData->paytm : '';
				//$userArray['otp'] =	$user->email ? $user->email : '';
				$userArray['image'] = ($userData->image) ? url('public/upload/profile') . '/' . $userData->image : '';
				...
				$userArray['email'] = ($userData->email) ? $userData->email : '';
				$userArray['phone'] = ($userData->phone) ? $userData->phone : '';
				$userArray['bank'] = $userData->bank ? $user->bank : '';
				$userArray['acount_no'] = $userData->acount_no ? $user->acount_no : '';

				$userArray['ifsc'] = $userData->ifsc ? $user->ifsc : '';
				$userArray['paytm'] =	$userData->paytm ? $user->paytm : '';
	...
	public function getQuestion(Request $request)
	{
		...
		$question = Quiz::where("date", date("Y-m-d"))->where("status", 1)->get();
		$ydate=date('Y-m-d',strtotime("-1 days"));	
		$questionyesdaydata = Quiz::where("date", $ydate)->where("status", 1)->get();
		...
		$result = array();
			...
			foreach ($question as $key => $value) {
				//echo  $value->id;die();
			$quizcheck = UserQuestion::where("quiz", $value->id)->where("user_id", $userId)->first();
			...
			$questionyesday = Question::where("quiz", $value->id)->where("date", date("Y-m-d"))->where("status", 1)->get();
			
			$result['today'][$key]['id'] = $value->id;
			$result['today'][$key]['title'] = $value->title;
			$result['today'][$key]['date'] = $value->date;
			$result['today'][$key]['issubmit'] = $issubmit;
			
			foreach ($questionyesday as $key1 => $value1) {
			$result['today'][$key]['question'][$key1]['id'] = $value1->id;
			$result['today'][$key]['question'][$key1]['question'] = $value1->question;
			$result['today'][$key]['question'][$key1]['option1'] = $value1->option1;
			$result['today'][$key]['question'][$key1]['option2'] = $value1->option2;
			$result['today'][$key]['question'][$key1]['option3'] = $value1->option3;
			$result['today'][$key]['question'][$key1]['option4'] = $value1->option4;
			}

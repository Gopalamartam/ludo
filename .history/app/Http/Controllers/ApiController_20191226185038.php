	...
	public function getQuestion(Request $request)
	{
		...
		if (!empty($questiontoday[0])) {
			...
			foreach ($questiontoday as $key => $value) {
				...
				$result['data'][0]['today'][$key]['id'] = $value->id;
				$result['data'][0]['today'][$key]['question'] = $value->question;
				$result['data'][0]['today'][$key]['option1'] = $value->option1;
				$result['data'][0]['today'][$key]['option2'] = $value->option2;
				$result['data'][0]['today'][$key]['option3'] = $value->option3;
				$result['data'][0]['today'][$key]['option4'] = $value->option4;
				$result['data'][0]['today'][$key]['quiz_id'] = $value->quiz;
				$open=0;
				if($quizess->date!=date("Y-m-d"))
				{
					$open=1;
				}
				$result['data'][0]['today'][$key]['quizeopen'] = $open;

				$result['data'][0]['today'][$key]['correct'] = $correct;
				$result['data'][0]['today'][$key]['ansgive'] = $ansGive;
				$result['data'][0]['today'][$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result['data'][0]['today'][$key]['date'] = $quizess->date;
			}

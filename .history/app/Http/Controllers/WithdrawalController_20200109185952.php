    ...
    public function prizedistribute(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (empty($allcheck)) {
                ...
                $question = Question::where("status", 1)->where("quiz", $quiz->id)->count();
                $completecorrect=array();
                if (!empty($question)) {


                    $userlist = UserQuestion::where("question_id", $question->id)->where("given_ans", $question->correct)->inRandomOrder()->get();

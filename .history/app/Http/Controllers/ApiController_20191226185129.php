	...
	public function getQuestion(Request $request)
	{
		...
		if (!empty($questiontoday[0])) {
			...
			foreach ($questiontoday as $key => $value) {
				...
				$result['data'][0][$key]['id'] = $value->id;
				$result['data'][0][$key]['question'] = $value->question;
				$result['data'][0][$key]['option1'] = $value->option1;
				$result['data'][0][$key]['option2'] = $value->option2;
				$result['data'][0][$key]['option3'] = $value->option3;
				$result['data'][0][$key]['option4'] = $value->option4;
				$result['data'][0][$key]['quiz_id'] = $value->quiz;
				$open=0;
				if($quizess->date!=date("Y-m-d"))
				{
					$open=1;
				}
				$result['data'][0][$key]['quizeopen'] = $open;

				$result['data'][0][$key]['correct'] = $correct;
				$result['data'][0][$key]['ansgive'] = $ansGive;
				$result['data'][0][$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result['data'][0][$key]['date'] = $quizess->date;
			}

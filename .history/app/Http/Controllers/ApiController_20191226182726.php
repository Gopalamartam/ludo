...
class ApiController extends Controller
{
	...
	public function myprofile(Request $request)
	{
		...
		if (!empty($userId)) {
			...
			/* 	User::where("id", $user->id)->update(['devicetoken' => $deviceToken]); */
			$data =$this->getuserDetail($userId);
			$msg = "My Profile";
	...
	public function getQuestion(Request $request)
	{
		...
		if(empty($todayquiz))
		{
			$todayquiz = Quiz::where("status", 1)->skip(0)->take(1)->first();
		}
		$ydate = date('Y-m-d', strtotime("-1 days"));
		$quizeyesterday = Quiz::where("date", $ydate)->where("status", 1)->first();
		if(empty($quizeyesterday))
		{
			$quizeyesterday = Quiz::where("status", 1)->skip(1)->take(1)->first();
		}
		...
		if (!empty($questiontoday[0])) {
			...
			foreach ($questiontoday as $key => $value) {
				...
				$correct = 0;
				$ansGive = 0;
				$quizess = Quiz::where("id", "=",  $value->quiz)->where("status", 1)->first();
				if (!empty($quizcheck)) {
				...
				$result['data']['today'][$key]['id'] = $value->id;
				$result['data']['today'][$key]['question'] = $value->question;
				$result['data']['today'][$key]['option1'] = $value->option1;
				$result['data']['today'][$key]['option2'] = $value->option2;
				$result['data']['today'][$key]['option3'] = $value->option3;
				$result['data']['today'][$key]['option4'] = $value->option4;
				$result['data']['today'][$key]['quiz_id'] = $value->quiz;
				$open=0;
				if($quizess->date!=date("Y-m-d"))
				{
					$open=1;
				}
				$result['data']['today'][$key]['quizeopen'] = $open;

				$result['data']['today'][$key]['correct'] = $correct;
				$result['data']['today'][$key]['ansgive'] = $ansGive;
				$result['data']['today'][$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result['data']['today'][$key]['date'] = $quizess->date;
			}
		}
		...
		if (!empty($questionyesterday[0])) {
			...
			foreach ($questionyesterday as $key => $value) {
				...
				$result1['data']['yesterday'][$key]['id'] = $value->id;
				$result1['data']['yesterday'][$key]['question'] = $value->question;
				$result1['data']['yesterday'][$key]['option1'] = $value->option1;
				$result1['data']['yesterday'][$key]['option2'] = $value->option2;
				$result1['data']['yesterday'][$key]['option3'] = $value->option3;
				$result1['data']['yesterday'][$key]['option4'] = $value->option4;
				$result1['data']['yesterday'][$key]['quiz_id'] = $value->quiz;
			
				$result1['data']['yesterday'][$key]['quizeopen'] = 1;
				
				$result1['data']['yesterday'][$key]['correct'] = $correct;
				$result1['data']['yesterday'][$key]['ansgive'] = $ansGive;
				$result1['data']['yesterday'][$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result1['data']['yesterday'][$key]['date'] = $quizess->date;
			}
		}
		$result2 = array();
		if (!empty($allother[0])) {
			foreach ($allother as $key1 => $value1) {
				...
				if ($requireQuestion <= $totalquestion) {
					...
					$result2['data']['old'][$key1]['quizid'] = $value1->id;
					$result2['data']['old'][$key1]['date'] = $value1->date;
					$tempArray = array();
					foreach ($data as $key => $value) {
						...
						$tempArray[$key]['quizeopen'] = 1;
						$tempArray[$key]['correct'] = $correct;
						$tempArray[$key]['ansgive'] = $ansGive;
						$tempArray[$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
						$tempArray[$key]['date'] = $quizess->date;
					}
					$result2['data']['old'][$key1]['question'] = $tempArray;
				}
			}
		}
		...
		return response()->json([
			'statusCode' => 200,
			'message' => $message,
			'data' => array("today" => $result, "yesterday" => $result1, "old" => $result2),
		]);
		/*else {
	...
	public function howplay(Request $request)
	{
		...
		if (!empty($start)) {
			...
			$offset = $start * 10;
			$limit = 10;
			$blogs = Blog::where("status", 1)->offset($offset)->limit($limit)->get();
			...
			foreach ($blogs as $key => $val) {
				...
				$result[$key]['description'] = $val->description ?  $val->description : '';
			
				$result[$key]['url'] = ($val->youtubeurl) ?  $val->youtubeurl : '';
	...
	public function myBatting(Request $request)
	{
		...
		if (!empty($batting)) {
			...
			foreach ($batting as $key => $value) {
				...
				$status=$totalcorrect."/".$totalquestion;
				if($notdec==1)
				{

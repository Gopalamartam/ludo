	...
	public function getQuestion(Request $request)
	{
		...
		if (!empty($questionyesterday[0])) {
			...
			foreach ($questionyesterday as $key => $value) {
				...
				$result['data']['yesterday'][$key]['id'] = $value->id;
				$result['data']['yesterday'][$key]['question'] = $value->question;
				$result['data']['yesterday'][$key]['option1'] = $value->option1;
				$result['data']['yesterday'][$key]['option2'] = $value->option2;
				$result['data']['yesterday'][$key]['option3'] = $value->option3;
				$result['data']['yesterday'][$key]['option4'] = $value->option4;
				$result['data']['yesterday'][$key]['quiz_id'] = $value->quiz;
			
				$result['data']['yesterday'][$key]['quizeopen'] = 1;
				
				$result['data']['yesterday'][$key]['correct'] = $correct;
				$result['data']['yesterday'][$key]['ansgive'] = $ansGive;
				$result['data']['yesterday'][$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result['data']['yesterday'][$key]['date'] = $quizess->date;
			}
		}
		$result2 = array();
		if (!empty($allother[0])) {
			foreach ($allother as $key1 => $value1) {
				...
				if ($requireQuestion <= $totalquestion) {
					...
					$result['data']['old'][$key1]['quizid'] = $value1->id;
					$result['data']['old'][$key1]['date'] = $value1->date;
					$tempArray = array();
					...
					$result['data']['old'][$key1]['question'] = $tempArray;
				}
			}
		}

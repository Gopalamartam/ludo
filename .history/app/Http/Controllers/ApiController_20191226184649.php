	...
	public function getQuestion(Request $request)
	{
		...
		if (!empty($questiontoday[0])) {
			...
			foreach ($questiontoday as $key => $value) {
				...
				$result['data'][$key]['question'] = $value->question;
				$result['data'][$key]['option1'] = $value->option1;
				$result['data'][$key]['option2'] = $value->option2;
				$result['data'][$key]['option3'] = $value->option3;
				$result['data'][$key]['option4'] = $value->option4;
				$result['data'][$key]['quiz_id'] = $value->quiz;
				$open=0;
				if($quizess->date!=date("Y-m-d"))
				{
					$open=1;
				}
				$result['data'][$key]['quizeopen'] = $open;

				$result['data'][$key]['correct'] = $correct;
				$result['data'][$key]['ansgive'] = $ansGive;
				$result['data'][$key]['givenOption'] = @$quizcheck->given_ans?$quizcheck->given_ans:0;
				$result['data'][$key]['date'] = $quizess->date;
			}
		...
		return response()->json([
			'statusCode' => 200,
			'message' => $message,
			'data' => array("today" => $return1, "yesterday" => $return2, "old" => $result2),
		]);
		/*else {
